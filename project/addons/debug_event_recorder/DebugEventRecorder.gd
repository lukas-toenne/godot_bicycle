...
export(float) var vector_thickness := 0.02
export(float) var vector_tip_length := 0.04
export(float) var spring_thickness := 0.02
export(float) var spring_piston_height := 0.02
export(float) var spring_piston_radius := 0.08
...
func _get_or_create_event(node: Node, key, on_create: FuncRef):
...
func record_vector(node: Node, key, start: Vector3, direction: Vector3):
	var vec = _get_or_create_event(node, key, funcref(self, "_make_vector"))
	...
	var z := x.cross(y)
	var height := max(direction.length() - vector_tip_length, 0.0)
	vec[0].transform = Transform(x * vector_thickness, y * height,            z * vector_thickness, start + y * height * 0.5)
	vec[1].transform = Transform(x * vector_thickness, y * vector_tip_length, z * vector_thickness, start + y * height)
...
func record_spring(node: Node, key, origin: Vector3, direction: Vector3, rest_length: float, length: float):
	var spring = _get_or_create_event(node, key, funcref(self, "_make_spring"))
	...
	if x.length_squared() < 1.0e-6:
		x = y.cross(Vector3(1, 0, 0)).normalized()
	var z := x.cross(y)
	spring[0].transform = Transform(x * spring_thickness,     y * rest_length,          z * spring_thickness,     origin + y * rest_length * 0.5)
	spring[1].transform = Transform(x * spring_piston_radius, y * spring_piston_height, z * spring_piston_radius, origin + y * (length - 0.5 * spring_piston_height))
